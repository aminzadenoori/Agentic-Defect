...
All baselines are single-agent and receive the same retrieved pool where applicable, isolating the effect of \emph{structure}: \emph{Zero-shot} (one call, brief rationale requested); \emph{Few-shot} ($k{=}5$ retrieved examples with their annotations, one call); and \emph{CoT few-shot} (same examples, each carrying a one-sentence reasoning chain, with an explicit step-by-step instruction, one call). Critically, \emph{no} method---baseline or MARQ---is told the weak word for the test requirement: each is given only the requirement text and must surface and reason about candidate ambiguous terms itself. Weak-word annotations appear in prompts only for the retrieved \emph{pool} examples, and identically for baselines and for MARQ's retriever. The baselines therefore solve exactly the same hint-free problem as MARQ and differ from it only in reasoning structure, which is what makes the comparison fair.
...
Following ICSE's encouragement to reflect on environmental impact, we note the energy profile of our approach; this is not a review criterion but is reported for transparency. We are candid about a tension: MARQ spends three to six model calls per prediction where a baseline spends one, so a naive reading is that structure costs more energy. Three factors bound that cost. On \emph{computation}, every model is a small ($\leq$7B) open language model running with greedy decoding on a single consumer GPU, so per-call energy is low. On \emph{training and deployment}, the method is entirely training-free---no fine-tuning, no large-model serving---and the only offline step (synthesizing demonstration reasoning) is cached once per fold, as are sentence embeddings for retrieval, avoiding recomputation. On \emph{design}, the pipeline short-circuits to a single call when the scanner finds no concern, caps the number of investigated concerns, and allows the critic at most one bounded revision round, keeping the call budget small and predictable. We log LLM calls and token counts per prediction and will report estimated energy per prediction in the full study, so that the accuracy gain from structure can be weighed explicitly against its footprint rather than hidden.
